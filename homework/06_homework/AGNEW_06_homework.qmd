...
#plot distance density by SEM condition
sim |> 
  ggplot(aes(x = Distance, color = Sem)) +
  geom_density() +
  theme_minimal()
...
## HERE DESCRIBE
# plenty of overlap between SEM conditions
# data has a long tail 
# clearly right-skewed
# Question from me: would it not be best to log transform this data?
# Differnt has highher narrower peak
# Similar flatter 
...
plot_normal(mean = 100, sd = 15) +
  xlim(0,214)

#actual mean SD of data
plot_normal(mean = 78, sd = 40) +
  xlim(0,214)

#descriptives from above
#77.60331	40.13778	3	214	
...
my_priors <- c(prior(normal(107,50), class = 'intercept'),
               # assumption no difference between mean of SEM and DIFF, and SD can be wide-ish/same as other 
               priod(normal(0,50), class= 'b', coef = 'SemSimilar'))

#model only three parameters: predictor, intercept, sigma (residual varience)
...
# Data peak aligned pretty desently with posteriors 
# The posteriors peaks are lower than the data
# The peak of the data is higher than like 99 of the posteriors
# the data is narrower 
# data has a right skewed bumb that doesnt replciatite in posteriors
#overall matches a lot better on the right than left
